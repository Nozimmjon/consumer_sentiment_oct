...
```{r}
tashkent_input_01 <- readxl::read_xlsx(here("data", "tashkent_2022_07.xlsx")) 
```
...
```{r}
tashkent_input_02 <- tashkent_input_01 %>% 
  select(-3) %>% 
  set_names(new_names) %>% 
...
```{r}
skim(tashkent_input_02)
```

```{r}
q1_reg <- reg_table(tashkent_input_02, q_1) %>% 
...
q2_reg <- reg_table(tashkent_input_02, q_2) %>% 
...
q3_reg <- reg_table(tashkent_input_02, q_3) %>% 
...
q4_reg <- reg_table(tashkent_input_02, q_4) %>% 
...
q5_reg <- reg_table(tashkent_input_02,q_5) %>% 
...
q6_reg <- reg_table(tashkent_input_02, q_6) %>% 
...
q7 <- reg_table(tashkent_input_02, q_7) %>% 
  mutate_at(vars(-district), as.double) %>% 
...
#Save Workbook
saveWorkbook(wb, "tashkent_indeks_2022_07.xlsx", overwrite = TRUE) 
```
