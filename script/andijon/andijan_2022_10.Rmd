...
```{r}
andijan_input_01 <- readxl::read_xlsx(here("data", "andijon_2022_10.xlsx")) 
```
...
```{r}
andijan_input_02 <- andijan_input_01 %>% 
  select(-3) %>% 
  set_names(new_names) %>% 
  ...
  drop_na(district) %>% 
  mutate(district = recode(district, "Бўз" = "Бўстон",
                           "Андижон тумани" = "Андижон т.",
                           "Избосган" = "Избоскан")) %>% 
  mutate(q_11 = recode(q_11, 
                       ...
                       "Газ таъминотидаги тез-тез узилишлар ;" = "Газ таъминотида узилишлар",
                       "Электр таъминоти узилиши билан боглик муаммо" = "Электр таъминотида узилишлар",
                       "Ўтин етишмаслиги/қимматлиги" = "Ўтин етишмаслиги/қимматлиги",
                       .default = "Бошқа")) %>%
```

```{r}
skim(andijan_input_02)
```

```{r}
q1_reg <- reg_table(andijan_input_02, q_1) %>% 
...
q2_reg <- reg_table(andijan_input_02, q_2) %>% 
...
q3_reg <- reg_table(andijan_input_02, q_3) %>% 
...
q4_reg <- reg_table(andijan_input_02, q_4) %>% 
...
q5_reg <- reg_table(andijan_input_02,q_5) %>% 
...
q6_reg <- reg_table(andijan_input_02, q_6) %>% 
...
q7 <- reg_table(andijan_input_02, q_7) %>% 
  mutate_at(vars(-district), as.double) %>% 
...
#Save Workbook
saveWorkbook(wb, "andijan_indeks_2022_10.xlsx", overwrite = TRUE) 
```
