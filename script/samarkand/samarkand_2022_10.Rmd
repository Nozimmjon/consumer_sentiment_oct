...
```{r}
samarkand_input_01 <- readxl::read_xlsx(here("data", "samarkand_2022_10.xlsx")) 
```
...
```{r}
samarkand_input_02 <- samarkand_input_01 %>% 
  select(-3) %>% 
  set_names(new_names) %>% 
  ...
  drop_na(district) %>% 
  mutate(district = recode(district, "Самарқанд тумани" = "Самарқанд", 
  "Самарқанд шаҳри" = "Самарқанд ш.",
  "Патдарғом" = "Пастдарғом",
  "Тайлоқ" = "Тойлоқ")) %>%
...
```{r}
skim(samarkand_input_02)
```

```{r}
q1_reg <- reg_table(samarkand_input_02, q_1) %>% 
...
q2_reg <- reg_table(samarkand_input_02, q_2) %>% 
...
q3_reg <- reg_table(samarkand_input_02, q_3) %>% 
...
q4_reg <- reg_table(samarkand_input_02, q_4) %>% 
...
q5_reg <- reg_table(samarkand_input_02,q_5) %>% 
...
q6_reg <- reg_table(samarkand_input_02, q_6) %>% 
...
q7 <- reg_table(samarkand_input_02, q_7) %>% 
  mutate_at(vars(-district), as.double) %>% 
...
#Save Workbook
saveWorkbook(wb, "samarkand_indeks_2022_10.xlsx", overwrite = TRUE) 
```
