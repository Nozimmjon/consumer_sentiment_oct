...
```{r}
navoiy_input_01 <- readxl::read_xlsx(here("data", "navoiy_2022_10.xlsx")) 
```
...
```{r}
navoiy_input_02 <- navoiy_input_01 %>% 
  select(-3) %>% 
  set_names(new_names) %>% 
...
```{r}
skim(navoiy_input_02)
```


```{r}
q1_reg <- reg_table(navoiy_input_02, q_1) %>% 
...
q2_reg <- reg_table(navoiy_input_02, q_2) %>% 
...
q3_reg <- reg_table(navoiy_input_02, q_3) %>% 
...
q4_reg <- reg_table(navoiy_input_02, q_4) %>% 
...
q5_reg <- reg_table(navoiy_input_02,q_5) %>% 
...
q6_reg <- reg_table(navoiy_input_02, q_6) %>% 
...
q7 <- reg_table(navoiy_input_02, q_7) %>% 
  mutate_at(vars(-district), as.double) %>% 
...
#Save Workbook
saveWorkbook(wb, "navoiy_indeks_2022_10.xlsx", overwrite = TRUE) 
```
