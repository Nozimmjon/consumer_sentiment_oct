...
# table_7 <- buxoro_input_02 %>% add_count(district) %>% 
...
# 
# write_xlsx(table_7, "muammolar_buxoro.xlsx")
...
#   cols_width(everything() ~ px(120)) %>%
#   my_theme_gt() %>%
#   gtsave('hokim_yordamchisi.png', path = here("results", "tables", "buxoro"))
# 
...
#   cols_width(everything() ~ px(120)) %>%
#   my_theme_gt() %>%
#   gtsave('yoshlar_yetakchisi.png', path = here("results", "tables", "buxoro"))
# 
# 
...
#   cols_width(everything() ~ px(120)) %>%
#   my_theme_gt() %>%
#   gtsave('ayollar_yetakchisi.png', path = here("results", "tables", "buxoro"))
...
#income groups
buxoro_input_02 %>% filter(income != "Жавоб беришдан бош тортди") %>% 
