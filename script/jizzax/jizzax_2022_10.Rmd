...
```{r}
jizzax_input_01 <- readxl::read_xlsx(here("data", "jizzax_2022_10.xlsx")) 
```
...
```{r}
jizzax_input_02 <- jizzax_input_01 %>% 
  select(-3) %>% 
  set_names(new_names) %>% 
  ...
  mutate(q_11 = str_replace_all(q_11, "(электропечка, пушка, кондиционер ва х.к.)", "")) %>% 
  mutate(q_11 = recode(q_11, 
                       "Муаммо йук" = "Муаммо йўқ",
                       "Кўмирнинг етишмаслиги/қимматлиги;" = "Кўмир етишмаслиги/қимматлиги",
                       "Суюлтирилган газни етказиб бериш билан боғлиқ муаммо/қимматлиги;" = "Суюлтирилган газ етказиб бериш",
                       ...
                       "Электр таъминоти узилиши билан боглик муаммо" = "Электр таъминотида узилиш",
                        "Электр таъминоти" = "Электр таъминотида узилиш",
                       "Ўтин етишмаслиги/қимматлиги" = "Ўтин етишмаслиги",
                       "Таббий газ <U+049B>имматлиги" = "Газ қимматлиги",
                       "Газнинг кимматлиги" = "Газ қимматлиги",
                       .default = "Бошқа"))
```

```{r}
skim(jizzax_input_02)
```

```{r}
q1_reg <- reg_table(jizzax_input_02, q_1) %>% 
...
q2_reg <- reg_table(jizzax_input_02, q_2) %>% 
...
q3_reg <- reg_table(jizzax_input_02, q_3) %>% 
...
q4_reg <- reg_table(jizzax_input_02, q_4) %>% 
...
q5_reg <- reg_table(jizzax_input_02,q_5) %>% 
...
q6_reg <- reg_table(jizzax_input_02, q_6) %>% 
...
q7 <- reg_table(jizzax_input_02, q_7) %>% 
  mutate_at(vars(-district), as.double) %>% 
...
#Save Workbook
saveWorkbook(wb, "jizzax_indeks_2022_10.xlsx", overwrite = TRUE) 
```
